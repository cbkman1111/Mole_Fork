...
namespace MEC
{
    public class Timing : MonoBehaviour
    {
...
#if UNITY_EDITOR
					// 에디터에서 플레이 버튼 정확한 활성화 타이밍 적용.
					if (UnityEditor.EditorApplication.isPlaying)
	                    DontDestroyOnLoad(instanceHome);

					// 싱글톤 인스턴스 생성시 scene에 저장되지 않도록 플래그 설정.
					instanceHome.hideFlags = HideFlags.DontSaveInEditor | HideFlags.DontSaveInBuild;
#else
                    DontDestroyOnLoad(instanceHome);
#endif
					#endregion
        ...
        {
			#region METAWORLD_CODE
			// 플레이 모드가 아닐 때 코루틴 호출하면 경고 표시하도록 개선.
#if UNITY_EDITOR
			if (false == Application.isPlaying &&
				(!segment.HasFlag(Segment.EditorSlowUpdate) && !segment.HasFlag(Segment.EditorUpdate)))
			{
				Debug.LogWarning("플레이중이 아닐 때 세그먼트 없이 코루틴을 호출했습니다. 의도일 경우 Segment를 입력하세요");
			}
#endif
			#endregion
        ...
        #region METAWORLD_CODE
        // 메타월드에서는 해당 메써드를 직접콜하지 않는다. 
        //		public static int KillCoroutines() -> protected static int KillCoroutines()
        protected static int KillCoroutines()
        {
            return _instance == null ? 0 : _instance.KillCoroutinesOnInstance();
        }
        #endregion // METAWORLD_CODE
        ...
        public static CoroutineHandle CallDelayed(float delay, System.Action action)
        {
	        #region METAWORLD_CODE
	        #if UNITY_EDITOR
	        var segment = !Application.isPlaying ? Segment.EditorUpdate : Segment.Update;
	        #else
	        var segment = Segment.Update;
			#endif
	        return action == null ? new CoroutineHandle() : RunCoroutine(Instance._DelayedCall(delay, action, null), segment);
	        #endregion
	        
	        // [2022-06-22] narlmay
	        // 에디터 모드에서도 CallDelayed() 호출을 하기 때문에 대응 코드 작성
